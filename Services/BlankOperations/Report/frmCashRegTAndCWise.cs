...
{
    public partial class frmCashRegTAndCWise : Form
    {
        ...
        public frmCashRegTAndCWise()
        {
            InitializeComponent();
        }

        public frmCashRegTAndCWise(string sTransactionDate, SqlConnection conn, string sTerminalId, string sStaffId)
        {
            InitializeComponent();
        ...
        private void frmCashRegTAndCWise_Load(object sender, EventArgs e)
        {
            reportViewer1.ProcessingMode = ProcessingMode.Local;
            LocalReport localReport = reportViewer1.LocalReport;
            localReport.ReportPath = "CashRegisterTerminalAndCashierWise.rdlc";
        ...
        {
            string sQuery = "GETCASHREGISTERTRANS_TerminalAndCashierWise";
            ...
            if (string.IsNullOrEmpty(sTerminal))
                command.Parameters.Add("@TERMINAL", DBNull.Value);
            else
                command.Parameters.Add("@TERMINAL", sTerminal);

            if (string.IsNullOrEmpty(sStaff))
                command.Parameters.Add("@STAFFID", DBNull.Value);
            else
                command.Parameters.Add("@STAFFID", sStaff);
        ...
        private void GetCollection(ref DataTable dtCol)
        {
            string sQuery = "GETCASHREGOTHCOLLECTION_TerminalAndCashierWise";
            SqlCommand command = new SqlCommand(sQuery, connection);
            command.CommandType = CommandType.StoredProcedure;
            command.CommandTimeout = 0;
            command.Parameters.Clear();
            ...
            if (string.IsNullOrEmpty(sTerminal))
                command.Parameters.Add("@TERMINALID", DBNull.Value);
            else
                command.Parameters.Add("@TERMINALID", sTerminal);
        ...
        {
            string sQuery = "GETCASHREGMOP_TerminalAndCashierWise";
            SqlCommand command = new SqlCommand(sQuery, connection);
            command.CommandType = CommandType.StoredProcedure;
            command.CommandTimeout = 0;
            command.Parameters.Clear();
        ...
        {
            string sQuery = "GETTENDERDENOMINATION_TerminalAndCashierWise";
            SqlCommand command = new SqlCommand(sQuery, connection);
            command.CommandType = CommandType.StoredProcedure;
            command.CommandTimeout = 0;
            command.Parameters.Clear();
        ...
        {
            string sQuery = "GETFDDETAILS_TerminalAndCashierWise";
            SqlCommand command = new SqlCommand(sQuery, connection);
            command.CommandType = CommandType.StoredProcedure;
            command.CommandTimeout = 0;
            command.Parameters.Clear();
        ...
        {
            string sQuery = "GETTENDERDECLARE_TerminalAndCashierWise";
            SqlCommand command = new SqlCommand(sQuery, connection);
            command.CommandType = CommandType.StoredProcedure;
            command.CommandTimeout = 0;
            command.Parameters.Clear();
